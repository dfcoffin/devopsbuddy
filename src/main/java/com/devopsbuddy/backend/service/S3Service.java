...
public class S3Service {
    ...
    public String storeProfileImage(MultipartFile uploadedFile, String username) {
        ...
        try {
            if (uploadedFile != null && !uploadedFile.isEmpty()) {
                ...
                LOG.info("Temporary file will be save to {}", tmpProfileImageFile.getAbsolutePath());
    ...
    /**
     * Store the given file name in S3 and return the key under which the file has been stored
    ...
    private String storeProfileImageToS3(File resource, String username) {
        ...
        if (null == rootBucketUrl) {
            LOG.error("The bucket {} does not exist and the application " +
                    "was not able to create it.  the image won't be sotred with the profile", rootBucketUrl);
        } else {
